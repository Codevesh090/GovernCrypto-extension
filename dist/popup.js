...
(() => {
  ...
  async function fetchAllActiveProposals(skip = 0) {
    const query = `{
    proposals(
      first: 40,
      skip: ${skip},
  ...
  async function fetchProposalsBySpace(spaceId, skip = 0) {
    const query = `
...
- If the proposal body is empty or under 20 words, respond with exactly: "No description provided for this proposal."`;
  ...
  async function generateSummary(proposalBody, apiKey, language = "en") {
...
${proposalBody}`
  ...
  async function getCachedSummary(proposalId, language = "en") {
    try {
  ...
  async function cacheSummary(proposalId, summary, language = "en") {
    try {
      const key = `${CACHE_PREFIX}${language}_${proposalId}`;
  ...
  function getVoiceForLanguage(language, selectedVoiceId) {
    const languageVoiceMap = {
      "hi": "nPczCjzI2devNBz1zQrb",
      // Hindi voice (Brian - multilingual, excellent Hindi)
      "es": "EXAVITQu4vr4xnSDxMaL",
      // Spanish voice (Sarah - native Spanish speaker)
      "fr": "ThT5KcBeYPX3keUQqHPh",
      // French voice (Dorothy - native French speaker)
      "de": "pNInz6obpgDQGcFmaJgB",
      // German voice (Adam - native German speaker)
      "pt": "XB0fDUnXU5powFXDhCwa",
      // Portuguese voice (Charlotte - native Portuguese)
      "zh": "onwK4e9ZLuTAKqWW03F9",
      // Chinese voice (Daniel - native Mandarin)
      "ja": "IKne3meq5aSn9XLyUdCD",
      // Japanese voice (Charlie - native Japanese)
      "ko": "bIHbv24MWmeRgasZH58o",
      // Korean voice (Clyde - native Korean)
      "ru": "nPczCjzI2devNBz1zQrb",
      // Russian voice (Brian - native Russian)
      "it": "XrExE9yKIg1WjnnlVkGX",
      // Italian voice (Matilda - native Italian)
      "nl": "flq6f7yk4E4fJM5XTYuZ",
      // Dutch voice (Michael - native Dutch)
      "tr": "yoZ06aMxZJJ28mfd3POQ",
      // Turkish voice (Sam - native Turkish)
      "vi": "AZnzlk1XvdvUeBnXmlld",
      // Vietnamese voice (Domi - native Vietnamese)
  ...
  function getLanguageName2(code) {
    const names = {
      "en": "English",
      "hi": "Hindi",
      "es": "Spanish",
  ...
  // src/speechNormalizer.ts
  function normalizeForSpeech(text, language) {
    let normalized = text;
    switch (language) {
      case "hi":
  ...
  function normalizeSpanish(text) {
    ...
    result = result.replace(/\$(\d+(?:,\d{3})*(?:\.\d{2})?)/g, (match, amount) => {
      const num = parseFloat(amount.replace(/,/g, ""));
      return numberToSpanishWords(num) + " d\xF3lares";
    });
    result = result.replace(/(\d+(?:\.\d+)?)%/g, (match, num) => {
  ...
  function normalizePortuguese(text) {
    ...
    result = result.replace(/\$(\d+(?:,\d{3})*(?:\.\d{2})?)/g, (match, amount) => {
      const num = parseFloat(amount.replace(/,/g, ""));
      return numberToPortugueseWords(num) + " d\xF3lares";
    });
    result = result.replace(/(\d+(?:\.\d+)?)%/g, (match, num) => {
  ...
  function numberToSpanishWords(num) {
    ...
    const teens = ["diez", "once", "doce", "trece", "catorce", "quince", "diecis\xE9is", "diecisiete", "dieciocho", "diecinueve"];
  ...
  function numberToGermanWords(num) {
    ...
    const ones = ["", "eins", "zwei", "drei", "vier", "f\xFCnf", "sechs", "sieben", "acht", "neun"];
    const teens = ["zehn", "elf", "zw\xF6lf", "dreizehn", "vierzehn", "f\xFCnfzehn", "sechzehn", "siebzehn", "achtzehn", "neunzehn"];
    const tens = ["", "", "zwanzig", "drei\xDFig", "vierzig", "f\xFCnfzig", "sechzig", "siebzig", "achtzig", "neunzig"];
    if (num < 10) return ones[num];
    if (num < 20) return teens[num - 10];
    if (num < 100) {
      const ten = Math.floor(num / 10);
  ...
  function numberToPortugueseWords(num) {
    if (num === 0) return "zero";
    const ones = ["", "um", "dois", "tr\xEAs", "quatro", "cinco", "seis", "sete", "oito", "nove"];
...
- Do NOT use romanized ${languageName} words like "samajh" - use Devanagari script or simple spoken equivalents.`;
        ...
        ttsInstructions = `
- Write ALL numbers in ${languageName} words: "1000" \u2192 "mil", "$1000" \u2192 "mil d\xF3lares"
- Write symbols in words: "$" \u2192 "d\xF3lares", "%" \u2192 "por ciento"
...
${currentProposalContext}`;
  ...
  async function castVote(proposalId, _spaceId, choiceIndex, _voterAddress) {
    await castVoteViaTab(proposalId, choiceIndex);
  }

  // src/languageStorage.ts
  ...
  var SUPPORTED_LANGUAGES = [
    ...
    { code: "hinglish", name: "Hinglish", flag: "\u{1F1EE}\u{1F1F3}" },
  ...
  // src/soundEffects.ts
  function playSoundEffect(effect, volume = 0.3) {
    try {
      ...
      switch (effect) {
        ...
        case "close":
          ...
          break;
        case "vote-cast":
          playChord(ctx, [523, 659, 784], volume, 0.5);
          return;
        // Early return since playChord handles cleanup
        case "deadline-warning":
          osc.frequency.setValueAtTime(900, ctx.currentTime);
          for (let i = 0; i < 3; i++) {
            gain.gain.setValueAtTime(volume, ctx.currentTime + i * 0.2);
            gain.gain.exponentialRampToValueAtTime(1e-3, ctx.currentTime + i * 0.2 + 0.1);
          }
          osc.start(ctx.currentTime);
  ...
  function playChord(ctx, frequencies, volume, duration) {
    ...
    setTimeout(() => ctx.close(), duration * 1e3 + 100);
  }
  async function areSoundEffectsEnabled() {
    const result = await chrome.storage.local.get("voiceSettings");
    return result.voiceSettings?.soundEffectsEnabled !== false;
  }
  async function playIfEnabled(effect, volume) {
  ...
  function applyTheme(theme) {
    ...
    const walletImg = document.getElementById("wallet-icon-img");
    if (walletImg) {
  ...
  function navigate(screen, data2) {
    appState.screen = screen;
    if (data2?.proposal) appState.selectedProposal = data2.proposal;
    renderCurrentScreen();
  }
  ...
  function renderProposalsList(proposals) {
    ...
    for (const p of safeProposals) {
      ...
      if (p.scores_total > 0 && p.percentages.length > 0) {
        const pairs = p.choices.map((c, i) => ({ choice: c, percent: p.percentages[i] || 0, score: p.scores[i] || 0, origIdx: i })).filter((item) => item.choice);
  ...
  function renderProposalDetail(proposal) {
    ...
    languageBtn.innerHTML = '<span id="current-lang-flag">\u{1F1EC}\u{1F1E7}</span> <span id="current-lang">EN</span> \u25BE';
    ...
    if (proposal.scores_total > 0 && proposal.percentages.length > 0) {
      const pairs = proposal.choices.map((c, i) => ({ choice: c, percent: proposal.percentages[i] || 0, score: proposal.scores[i] || 0, idx: i })).filter((item) => item.choice);
      const sorted = [...pairs].sort((a, b) => b.percent - a.percent);
      const colorMap = /* @__PURE__ */ new Map();
      sorted.forEach((item, rank) => {
  ...
  async function loadProposalsByTab(forceReload = false) {
    ...
    if (!forceReload && appState.proposalsSkip === 0 && lastFetchTime && lastFetchedTab === appState.activeTab && Date.now() - lastFetchTime < CACHE_TTL_MS2) {
  ...
  async function loadAISummary(proposal, languageCode) {
    resetSummarySection();
...
"${proposal.title}"?`);
  ...
  async function closeVoiceSettings() {
    const modal = document.getElementById("voice-settings-modal");
    if (!modal) return;
  ...
  async function saveVoiceSettingsFromModal() {
    const voiceSelect = document.getElementById("voice-select");
    const speedSlider = document.getElementById("voice-speed-slider");
    const soundSwitch = document.getElementById("sound-toggle-switch");
  ...
  async function handleVoiceButtonClick() {
    ...
    const { promise, stop } = recordWithSpeechAPI(
      ...
      3e3
      // 3 seconds of silence before stopping (reduced for better responsiveness)
    );
    stopRecording = stop;
    let transcript;
    try {
  ...
  async function initialize() {
    ...
    document.addEventListener("click", async (e) => {
      ...
      if (target.closest("#language-btn")) {
        const dropdown2 = document.getElementById("language-dropdown");
        if (dropdown2) {
          dropdown2.classList.toggle("show");
          await playIfEnabled(dropdown2.classList.contains("show") ? "open" : "close");
        }
        e.stopPropagation();
        return;
      }
